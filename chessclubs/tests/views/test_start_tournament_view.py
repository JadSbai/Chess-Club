"""Tests of start tournament view"""
...
class StartTournamentViewTestCase(TestCase, AssertHTMLMixin):
    """Test Suites of start_tournament view"""
    ...
    def setUpTestData(cls):
        ...
        cls.url = reverse('start_tournament', kwargs={'club_name':cls.club.name, 'tournament_name': cls.tournament.name})
    ...
    def test_start_tournament_url(self):
        self.assertEqual(self.url, f'/{self.club.name}/tournament/{self.tournament.name}/start_tournament/')

    def test_successful_start_tournament(self):
    ...
    def test_start_tournament_of_wrong_tournament(self):
    ...
    def test_wrong_club_start_tournament(self):
    ...
    def test_start_tournament_when_tournament_started(self):
    ...
    def test_non_participant_cannot_start_tournament(self):
        self.client.login(email=self.member.email, password='Password123')
        response = self.client.get(self.url, follow=True)
        self._assert_response_redirect(response)
    ...
    def test_non_logged_in_redirects(self):
        ...
        redirect_url = reverse_with_next('log_in', reverse('start_tournament', kwargs={'club_name':self.club.name, 'tournament_name': self.tournament.name}))
