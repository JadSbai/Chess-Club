...
class SmallPoolModelTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.player3 = Player.objects.get(pk=6)
        self.player4 = Player.objects.get(pk=7)
        ...
        self.pool = Pool.objects.get(pk=1)
        self.pool.create_matches()

    def test_create_matches(self):
        num = self.pool.get_players_count()
        pool_matches = self.pool.pool_matches.all()
        self.assertEqual(pool_matches.count(), (num * (num - 1)) / 2)
        for player in self.pool.get_players():
            for other_player in self.pool.get_players():
                if player != other_player:
                    count = 0
                    for match in self.pool.pool_matches.all():
    ...
    def test_enter_all_results(self):
        players = self.pool.get_players()
        match1 = self._get_match(self.player1, self.player2)
        self.pool.enter_result(match1, True, self.player1)

        match2 = self._get_match(self.player3, self.player1)
        self.pool.enter_result(match2, True, self.player3)

        match3 = self._get_match(self.player1, self.player4)
        self.pool.enter_result(match3, True, self.player1)

        match4 = self._get_match(self.player2, self.player3)
        self.pool.enter_result(match4, False)

        match5 = self._get_match(self.player2, self.player4)
        self.pool.enter_result(match5, True, self.player2)

        match6 = self._get_match(self.player3, self.player4)
        self.pool.enter_result(match6, True, self.player3)
        ...
        self.assertTrue(self.pool.all_matches_played)
